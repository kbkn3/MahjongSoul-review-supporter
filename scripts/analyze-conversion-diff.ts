...
class ConversionDiffAnalyzer {
  ...
  private v131Dir: string
  private v140Dir: string
  ...
  private getTestFiles(): string[] {
    ...
    // 両バージョンに存在するファイルのみ
    return v131Files.filter((f: string) => v140Files.includes(f))
  }

  /**
   * 個別ファイル分析
   */
  private analyzeFile(filename: string): FileAnalysis {
   ...
   * 1.3.1ファイル分析
   */
  private analyzeV131File(filename: string, analysis: FileAnalysis): void {
   ...
   * 1.4.0ファイル分析
   */
  private analyzeV140File(filename: string, analysis: FileAnalysis): void {
    ...
    if (existsSync(jsonPath)) {
      ...
      try {
        ...
        const data: GameData = JSON.parse(jsonContent)
   ...
   * 差分分析
   */
  private analyzeDifferences(analysis: FileAnalysis): void {
    // URL数の差分
    analysis.differences.urlCountDiff = analysis.v140.urlCount - analysis.v131.urlCount
    
    if (analysis.differences.urlCountDiff !== 0) {
      analysis.differences.issues.push(
        `URL count mismatch: v1.3.1=${analysis.v131.urlCount}, v1.4.0=${analysis.v140.urlCount}`
      )
    }
   ...
   * URL内容比較
   */
  private compareUrls(analysis: FileAnalysis): void {
   ...
   * URLからJSONパラメーターを抽出
   */
  private extractJsonParam(url: string): string | null {
    try {
   ...
   * 個別ファイルレポート出力
   */
  private printFileReport(analysis: FileAnalysis): void {
   ...
   * 総合レポート出力
   */
  private printSummaryReport(results: FileAnalysis[]): void {
   ...
   * 詳細レポートをファイルに出力
   */
  private writeDetailedReport(results: FileAnalysis[]): void {
