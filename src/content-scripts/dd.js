...
function checkscene(scene) {
  return (
    scene &&
    ((scene.Inst && scene.Inst._enable) || (scene._Inst && scene._Inst._enable))
  );
}
...
const pad_right = (a, l, f) =>
  !Array.from({ length: l - a.length }).map((_) => a.push(f)) || a;
...
function deaka(til) {
  ...
  if (5 == ~~(til / 10)) return 10 * (til % 10) + ~~(til / 10);
...
function parsehule(h, kyoku) {
    ...
    if (h.zimo) {
      ...
      if (h.qinjia) {
        ...
        delta[liableseat] -=
          ...
          tlround((1 / 2) * liablefor * YSCORE[OYA][KO]); // 1? only paying back other ko
        delta.forEach((e, i) => {
        ...
        delta[liableseat] -=
          ...
          tlround((1 / 2) * liablefor * YSCORE[KO][KO]); //^^same 1st, but ko
        delta.forEach((e, i) => {
          if (liableseat != i && h.seat != i && kyoku.nplayers >= i) {
  ...
  //append point symbol
  points +=
    RUNES.points[JPNAME] + (h.zimo && h.qinjia ? RUNES.all[NAMEPREF] : "");
  ...
  if (h.yiman)
    ...
    ALLOW_KIRIAGE &&
    ((4 == h.count && 30 == h.fu) || (3 == h.count && 60 == h.fu))
  )
    //kiriage
    res.push((SHOWFU ? fuhan : "") + RUNES.kiriagemangan[NAMEPREF] + points);
  //ordinary hand
  else res.push(fuhan + points);

  h.fans.forEach((e) =>
    res.push(
      (JPNAME == NAMEPREF
        ? cfg.fan.fan.map_[e.id].name_jp
        : cfg.fan.fan.map_[e.id].name_en) +
        "(" +
...
kyoku.countpao = function (tile, owner, feeder) {
  ...
  if (WINDS.includes(tile)) {
    if (4 == ++this.nowinds[owner]) this.paowind = feeder;
  } else if (DRAGS.includes(tile)) {
    if (3 == ++this.nodrags[owner]) this.paodrag = feeder;
  }
    ...
    switch (e.constructor.name) {
      ...
      case "RecordAnGangAddGang": {
        ...
        switch (e.type) {
          case 3: {
            ...
            let ankantiles = kyoku.haipais[e.seat]
              .filter((t) => (deaka(t) == deaka(til) ? true : false))
              .concat(
                kyoku.draws[e.seat].filter((t) =>
                  deaka(t) == deaka(til) ? true : false
                )
              );
      ...
      case "RecordLiuJu": {
        ...
        if (1 == e.type)
          ...
          //TODO: actually get the type code
          entry.push([RUNES.suuchariichi[NAMEPREF]]); //4 riichi
        else if (4 <= kyoku.nkan)
          //TODO: actually get type code
          entry.push([
            RUNES.suukaikan[NAMEPREF],
          ]); //4 kan, potentially false positive on 3 ron with 4 kans
        else entry.push([RUNES.sanchahou[NAMEPREF]]); //3 ron - can't actually get this in mjs
      ...
      case "RecordNoTile": {
        ...
        if (
          ...
          e.scores.forEach((f) =>
            f.delta_scores.forEach((g, i) => (delta[i] += g))
        ...
        //normal ryuukyoku
        else entry.push([RUNES.ryuukyoku[NAMEPREF], delta]);
        log.push(entry);
...
function parse(record) {
  ...
  //rule display
  if (3 == nplayers && JPNAME == NAMEPREF) ruledisp += RUNES.sanma[JPNAME];
  if (record.head.config.meta.mode_id)
    //ranked or casual
    ruledisp +=
      JPNAME == NAMEPREF
        ? cfg.desktop.matchmode.map_[record.head.config.meta.mode_id]
            .room_name_jp
        : cfg.desktop.matchmode.map_[record.head.config.meta.mode_id]
            .room_name_en;
    ...
    TSUMOLOSSOFF =
      3 == nplayers ? !record.head.config.mode.detail_rule.have_zimosun : false;
  ...
  if (
    ...
    !record.head.config.mode.detail_rule.dora_count
  ) {
    if (JPNAME != NAMEPREF) ruledisp += RUNES.nored[NAMEPREF];
    res["rule"] = { disp: ruledisp, aka53: 0, aka52: 0, aka51: 0 };
  } else {
    if (JPNAME == NAMEPREF) ruledisp += RUNES.red[JPNAME];
    res["rule"] = {
      disp: ruledisp,
      aka53: 1,
      aka52: 4 == nakas ? 2 : 1,
  ...
  // scores
  let scores = record.head.result.players.map((e) => [
    e.seat,
    e.part_point_1,
    e.total_point / 1000,
  ]);
  ...
  app.NetAgent.sendReq2Lobby(
    ...
    function (i, record) {
      ...
      download(
        //default filename
        (
          new Date(record.head.end_time * 1000).toLocaleDateString() +
          "_" +
          results["rule"]["disp"] +
          ".json"
        ).replace(/[ \/]/g, "_"),
