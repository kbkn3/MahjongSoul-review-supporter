...
/**
 * Akochan reviewerの形式になったjsonから表示用のデータを取り出す。
 * @param {*} message 牌譜データのjson
 */
export const convertAkochanToDisplay = (message: any) => {
  const kyoku_array: kyokuDisplay[] = []
  for (let i = 0; i < message.log.length; i++) {
    let kyoku: kyokuDisplay = {
      id: i,
      //場風
    ...
    kyoku_array.push(kyoku)
  }
  return kyoku_array
}

//雀魂の牌譜jsonを天鳳形式に変換
export const soul2naga = (results: any) => {
